"""
simpan_model_sektor.py
Training dengan fitur komoditas domain-specific per sektor
Setiap sektor dapat fitur komoditas yang paling relevan
"""
...
KOMODITAS_SEKTOR = {
    "tambang"      : ["coal","oil","gold"],
    "perbankan"    : ["gold","coal","oil"],
    "konsumer"     : ["gandum","beras","jagung","kedelai","cpo"],
    "agribisnis"   : ["cpo","beras","kedelai","coal","oil"],
    "energi"       : ["oil","coal"],
    "properti"     : ["gold","beras","coal","oil"],
    "ritel"        : ["beras","gandum","jagung","kedelai"],
    "kesehatan"    : ["gold"],
    "media"        : ["gold","oil"],
    "telekomunikasi": ["gold","coal","oil"],
    "lainnya"      : ["coal","oil","gold","beras","gandum"],
}

# Muat semua komoditas
print("Muat komoditas...")
KOM_PATH = "data/komoditas/KOMODITAS_GABUNGAN.csv"
df_kom = pd.read_csv(KOM_PATH, index_col=0)
df_kom.index = pd.to_datetime(df_kom.index)
print(f"  {len(df_kom.columns)} fitur komoditas tersedia")
...
SEMUA_FITUR = [c for c in df_all.columns if c not in EXCLUDE]

SD_FITUR = ["vol_spike","akumulasi","distribusi","mfi","cmf","momentum_5d","momentum_20d","is_breakout"]

def get_fitur_sektor(sektor, semua_fitur):
    kom_relevan = KOMODITAS_SEKTOR.get(sektor, ["coal","oil","gold"])
    fitur_kom = []
    for k in kom_relevan:
        fitur_kom += [f for f in semua_fitur if f.startswith(k + "_")]
    fitur_teknikal = [f for f in semua_fitur
                      if not any(f.startswith(k+"_") for k in
                                 ["coal","oil","gold","cpo","beras","gandum","jagung","kedelai"])]
    return fitur_teknikal + fitur_kom

# ── Training per sektor ───────────────────────────────────────
models = {}
print("\nTraining per sektor dengan fitur domain-specific...")
total_acc = []
...
for sektor in sorted(df_all["sektor"].unique()):
    ...
    fitur_sektor = get_fitur_sektor(sektor, SEMUA_FITUR)
...
with open("models/models_sektor.pkl", "wb") as f:
    pickle.dump(models, f)
...
print("\nPerbandingan:")
print(f"  Model sebelumnya      : 58.74%")
print(f"  Model domain-specific : {avg:.2%}")
delta = avg - 0.5874
